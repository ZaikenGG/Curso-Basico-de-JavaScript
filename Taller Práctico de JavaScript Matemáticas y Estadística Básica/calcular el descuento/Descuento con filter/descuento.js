...
function calculaDescuento(evento) {
  ...
  const validarCupon = couponList.filter((codigo) => coupon === codigo.name);

  if (validarCupon.length > 0) {
    // Si se encontró un cupón válido
    newDiscount = validarCupon[0].discount; // Obtener el descuento correspondiente
  } else {
